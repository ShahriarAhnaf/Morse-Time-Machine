...
int main(void)
{
    ...
    InitializePin(GPIOA, p1_green_led | p1_red_led | p2_green_led | p2_red_led, GPIO_MODE_OUTPUT_PP, GPIO_NOPULL, 0);  // leds    
    InitializePin(GPIOB, p1_button | p2_button, GPIO_MODE_INPUT, GPIO_PULLDOWN, 0);  // button
    InitializePin(GPIOB,  p1_buzzer | p2_buzzer, GPIO_MODE_OUTPUT_PP, GPIO_NOPULL, 0);
...
#ifdef GAME
struct morseString *sos = char_to_morse("SOS", 3);

//testing with single chars to increase testing speed
// questions 
struct morseString *year = char_to_morse("YEAR",4);
struct morseString *location = char_to_morse("LOC",3);
struct morseString *ship = char_to_morse("SHIP",4);

//answers
struct morseString *year_ans = char_to_morse("155",3);
struct morseString *location_ans = char_to_morse("CAN",3);
struct morseString *ship_ans = char_to_morse("E7",2);
   

// testing numbers in morse
//
// while(true)
// {
//     if(get_user_input(year_ans, p2_button, p2_buzzer)){
//         SerialPuts("deez");
//     }
// }
  // wait for the user to input SOS
   while(true){
       if (get_user_input(sos, p1_button, p1_buzzer)){
           SerialPuts("\nW");
           print_morse(sos, p2_green_led);
           break;
       }
       else {
           print_morse(sos, p1_red_led);
        }
   }
    // the game of three questions
    SerialPuts("first question\n");
    ask_question(1, year, year); //question
    HAL_Delay(300);
    ask_question(0, year, year_ans); // asnr
    blink_LED(300, p1_green_led);
    SerialPuts("second question\n");
    //communicate answer to the other player 
    print_morse(year_ans, p2_green_led);
    ask_question(1, location, location);
    HAL_Delay(300);
    ask_question(0, location, location_ans);
    blink_LED(300, p1_green_led);
    SerialPuts("third question\n");
    print_morse(location_ans, p2_green_led);
    ask_question(1, ship, ship);
    ask_question(0, ship, ship_ans);

    HAL_GPIO_TogglePin(GPIOA, p1_green_led);
    HAL_GPIO_TogglePin(GPIOA, p2_green_led);
}
#endif
    ...
    InitializePin(GPIOA, GPIO_PIN_7, GPIO_MODE_AF_PP, GPIO_NOPULL, GPIO_AF1_TIM2); // connect the LED to the timer output
